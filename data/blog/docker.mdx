...
docker run <image> e.g. docker run hello-world
...
An image is a filesystem snapshot
An image also contains a startup command
...
- `docker exec -it <image> npm run test`
...
1. Build image
2. Create instance of container from image (run time is when container is started up)
...
I should also cop package-lock.json or yarn.lock on the same line as package.json to lock in the dep versions!
